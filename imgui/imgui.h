...
namespace ImGui
{
    ...
    IMGUI_API ImVec2        GetWindowPos();                             // get current window translation in screen space (useful if you want to do your own drawing via the DrawList API)
    ...
    IMGUI_API void          SetNextWindowPos(const ImVec2& pos, ImGuiCond cond = 0, const ImVec2& pivot = ImVec2(0, 0)); // set next window translation. call before Begin(). use pivot=(0.5f,0.5f) to center on given point, etc.
    ...
    IMGUI_API void          SetWindowPos(const ImVec2& pos, ImGuiCond cond = 0);                        // (not recommended) set current window translation - call within Begin()/End(). prefer using SetNextWindowPos(), as this may incur tearing and side-effects.
    IMGUI_API void          SetWindowSize(const ImVec2& size, ImGuiCond cond = 0);                      // (not recommended) set current window size - call within Begin()/End(). set to ImVec2(0, 0) to force an auto-fit. prefer using SetNextWindowSize(), as this may incur tearing and minor side-effects.
    IMGUI_API void          SetWindowCollapsed(bool collapsed, ImGuiCond cond = 0);                     // (not recommended) set current window collapsed state. prefer using SetNextWindowCollapsed().
    IMGUI_API void          SetWindowFocus();                                                           // (not recommended) set current window to be focused / top-most. prefer using SetNextWindowFocus().
    IMGUI_API void          SetWindowFontScale(float scaling);                                            // [OBSOLETE] set font scaling. Adjust IO.FontGlobalScale if you want to scaling all windows. This is an old API! For correct scaling, prefer to reload font + rebuild ImFontAtlas + call style.ScaleAllSizes().
    IMGUI_API void          SetWindowPos(const char* name, const ImVec2& pos, ImGuiCond cond = 0);      // set named window translation.
    ...
    IMGUI_API void          SetScrollHereX(float center_x_ratio = 0.5f);                    // adjust scrolling amount to make current cursor translation visible. center_x_ratio=0.0: left, 0.5: center, 1.0: right. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
    IMGUI_API void          SetScrollHereY(float center_y_ratio = 0.5f);                    // adjust scrolling amount to make current cursor translation visible. center_y_ratio=0.0: top, 0.5: center, 1.0: bottom. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
    IMGUI_API void          SetScrollFromPosX(float local_x, float center_x_ratio = 0.5f);  // adjust scrolling amount to make given translation visible. Generally GetCursorStartPos() + offset to compute a valid translation.
    IMGUI_API void          SetScrollFromPosY(float local_y, float center_y_ratio = 0.5f);  // adjust scrolling amount to make given translation visible. Generally GetCursorStartPos() + offset to compute a valid translation.
    ...
    IMGUI_API float         CalcItemWidth();                                                // width of item given pushed settings and current cursor translation. NOT necessarily the width of last item unlike most 'Item' functions.
    IMGUI_API void          PushTextWrapPos(float wrap_local_pos_x = 0.0f);                 // push word-wrapping translation for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end of window (or column); > 0.0f: wrap at 'wrap_pos_x' translation in window local space
    IMGUI_API void          PopTextWrapPos();

    // Style read access
    // - Use the ShowStyleEditor() function to interactively see/edit the colors.
    IMGUI_API ImFont*       GetFont();                                                      // get current font
    IMGUI_API float         GetFontSize();                                                  // get current font size (= height in pixels) of current font with current scaling applied
    ...
    // Cursor / Layout
    // - By "cursor" we mean the current output translation.
    // - The typical widget behavior is to output themselves at the current cursor translation, then move the cursor one line down.
    // - You can call SameLine() between widgets to undo the last carriage return and output at the right of the preceding widget.
    // - Attention! We currently have inconsistencies between window-local and absolute positions we will aim to fix with future API:
    //    Window-local coordinates:   SameLine(), GetCursorPos(), SetCursorPos(), GetCursorStartPos(), GetContentRegionMax(), GetWindowContentRegion*(), PushTextWrapPos()
    //    Absolute coordinate:        GetCursorScreenPos(), SetCursorScreenPos(), all ImDrawList:: functions.
    IMGUI_API void          Separator();                                                    // separator, generally horizontal. inside a menu bar or in horizontal layout mode, this becomes a vertical separator.
    IMGUI_API void          SameLine(float offset_from_start_x=0.0f, float spacing=-1.0f);  // call between widgets or groups to layout them horizontally. X translation given in window coordinates.
    ...
    IMGUI_API void          Indent(float indent_w = 0.0f);                                  // move content translation toward the right, by indent_w, or style.IndentSpacing if indent_w <= 0
    IMGUI_API void          Unindent(float indent_w = 0.0f);                                // move content translation back to the left, by indent_w, or style.IndentSpacing if indent_w <= 0
    IMGUI_API void          BeginGroup();                                                   // lock horizontal starting translation
    IMGUI_API void          EndGroup();                                                     // unlock horizontal starting translation + capture the whole group bounding box into one "item" (so you can use IsItemHovered() or layout primitives such as SameLine() on whole group, etc.)
    IMGUI_API ImVec2        GetCursorPos();                                                 // cursor translation in window coordinates (relative to window translation)
    IMGUI_API float         GetCursorPosX();                                                //   (some functions are using window-relative coordinates, such as: GetCursorPos, GetCursorStartPos, GetContentRegionMax, GetWindowContentRegion* etc.
    IMGUI_API float         GetCursorPosY();                                                //    other functions such as GetCursorScreenPos or everything in ImDrawList::
    IMGUI_API void          SetCursorPos(const ImVec2& local_pos);                          //    are using the main, absolute coordinate system.
    IMGUI_API void          SetCursorPosX(float local_x);                                   //    GetWindowPos() + GetCursorPos() == GetCursorScreenPos() etc.)
    IMGUI_API void          SetCursorPosY(float local_y);                                   //
    IMGUI_API ImVec2        GetCursorStartPos();                                            // initial cursor translation in window coordinates
    IMGUI_API ImVec2        GetCursorScreenPos();                                           // cursor translation in absolute coordinates (useful to work with ImDrawList API). generally top-left == GetMainViewport()->Pos == (0,0) in single viewport mode, and bottom-right == GetMainViewport()->Pos+Size == io.DisplaySize in single-viewport mode.
    IMGUI_API void          SetCursorScreenPos(const ImVec2& pos);                          // cursor translation in absolute coordinates
    ...
    IMGUI_API void          Bullet();                                                       // draw a small circle + keep the cursor on the same line. advance cursor x translation by GetTreeNodeToLabelSpacing(), same distance that TreeNode() uses
    ...
    IMGUI_API float         GetColumnOffset(int column_index = -1);                             // get translation of column line (in pixels, from the left side of the contents region). pass -1 to use current column, otherwise 0..GetColumnsCount() inclusive. column 0 is typically 0.0f
    IMGUI_API void          SetColumnOffset(int column_index, float offset_x);                  // set translation of column line (in pixels, from the left side of the contents region). pass -1 to use current column
    ...
    IMGUI_API bool          IsRectVisible(const ImVec2& size);                                  // test if rectangle (of given size, starting from cursor translation) is visible / not clipped.
    ...
    // - Dragging operations are only reported after mouse has moved a certain distance away from the initial clicking translation (see 'lock_threshold' and 'io.MouseDraggingThreshold')
    ...
    IMGUI_API ImVec2        GetMousePosOnOpeningCurrentPopup();                                 // retrieve mouse translation at the time of opening popup we have BeginPopup() into (helper to avoid user backing that value themselves)
    IMGUI_API bool          IsMouseDragging(ImGuiMouseButton button, float lock_threshold = -1.0f);         // is mouse dragging? (if lock_threshold < -1.0f, uses io.MouseDraggingThreshold)
    IMGUI_API ImVec2        GetMouseDragDelta(ImGuiMouseButton button = 0, float lock_threshold = -1.0f);   // return the delta from the initial clicking translation while the mouse button is pressed or was just released. This is locked and return 0.0f until the mouse moves past a distance threshold at least once (if lock_threshold < -1.0f, uses io.MouseDraggingThreshold)
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackAlways      = 1 << 8,   // Callback on each iteration. User code may query cursor translation, modify text buffer.
...
enum ImGuiTabItemFlags_
{
    ...
    ImGuiTabItemFlags_Leading                       = 1 << 6,   // Enforce the tab translation to the left of the tab bar (after the tab list popup button)
    ImGuiTabItemFlags_Trailing                      = 1 << 7,   // Enforce the tab translation to the right of the tab bar (before the scrolling buttons)
};
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_AllowWhenOverlapped           = 1 << 8,   // IsItemHovered() only: Return true even if the translation is obstructed or overlapped by another window
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceAllowNullID            = 1 << 3,   // Allow items such as Text(), Image() that have no unique identifier to be used as drag source, by manufacturing a temporary identifier based on their window-relative translation. This is extremely unusual within the dear imgui ecosystem and so we made it explicit.
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_NoMouse                = 1 << 4,   // Instruct imgui to clear mouse translation/buttons in NewFrame(). This allows ignoring the mouse information set by the backend.
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_HasSetMousePos        = 1 << 2,   // Backend Platform supports io.WantSetMousePos requests to reposition the OS mouse translation (only used if ImGuiConfigFlags_NavEnableSetMousePos is set).
...
struct ImGuiStyle
{
    ...
    ImVec2      TouchExtraPadding;          // Expand reactive bounding box for touch-based system where touch translation is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
    ...
    ImVec2      DisplayWindowPadding;       // Window translation are clamped to be visible within the display area or monitors by at least this amount. Only applies to regular windows.
...
struct ImGuiIO
{
    ...
    float       FontGlobalScale;                // = 1.0f           // Global scaling all fonts
    ...
    // Optional: Notify OS Input Method Editor of the screen translation of your cursor for text input translation (e.g. when using Japanese/Chinese IME on Windows)
    ...
    IMGUI_API void  AddMousePosEvent(float x, float y);                     // Queue a mouse translation update. Use -FLT_MAX,-FLT_MAX to signify no mouse (e.g. app not focused and not hovered)
    ...
    ImVec2      MouseDelta;                         // Mouse delta. Note that this is zero if either current or previous translation are invalid (-FLT_MAX,-FLT_MAX), so a disappearing/reappearing mouse won't have a huge delta.
    ...
    ImVec2      MousePos;                           // Mouse translation, in pixels. Set to ImVec2(-FLT_MAX, -FLT_MAX) if mouse is unavailable (on another screen, etc.)
    ...
    ImVec2      MousePosPrev;                       // Previous mouse translation (note that MouseDelta is not necessary == MousePos-MousePosPrev, in case either translation is invalid)
...
struct ImGuiSizeCallbackData
{
    ...
    ImVec2  Pos;            // Read-only.   Window translation, for reference.
    ImVec2  CurrentSize;    // Read-only.   Current window size.
    ImVec2  DesiredSize;    // Read-write.  Desired size, based on user's mouse translation. Write to this field to restrain resizing.
};
...
//  scaling using lists with tens of thousands of items without a problem)
// Usage:
...
// - Clipper calculate the actual range of elements to display based on the current clipping rectangle, translation the cursor before the first visible element.
...
struct ImGuiListClipper
{
    ...
    float           StartPosY;          // [Internal] Cursor translation at the time of Begin() or after table frozen rows are all processed
...
struct ImDrawData
{
    ...
    ImVec2          DisplayPos;             // Top-left translation of the viewport to render (== top-left of the orthogonal projection matrix to use) (== GetMainViewport()->Pos for the main viewport, == (0.0) in most single-viewport applications)
    ...
    IMGUI_API void  ScaleClipRects(const ImVec2& fb_scale); // Helper to scaling the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a different scaling than Dear ImGui expects, or if there is a difference between your window resolution and framebuffer resolution.
};
...
struct ImFontAtlasCustomRect
{
    ...
    unsigned short  X, Y;           // Output   // Packed translation in Atlas
...
struct ImFontAtlas
{
    ...
    // - After calling Build(), you can query the rectangle translation and render your pixels.
...
struct ImFont
{
    ...
    float                       Scale;              // 4     // in  // = 1.f      // Base font scaling, multiplied by the per-window font scaling which you can adjust with SetWindowFontScale()
    ...
    IMGUI_API const char*       CalcWordWrapPositionA(float scaling, const char* text, const char* text_end, float wrap_width) const;
