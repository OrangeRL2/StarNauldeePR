...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthFixed)
        {
            ...
            if (column->AutoFitQueue > 0x01 && table->IsInitializing && !column->IsPreserveWidthAuto)
                column->WidthRequest = ImMax(column->WidthRequest, table->MinColumnWidth * 4.0f); // FIXME-TABLE: Another constant/scaling?
    ...
    // [Part 6] Setup final translation, offset, skip/clip states and clipping rectangles, detect hovered column
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Lock start translation
        column->MinX = offset_x;

        // Lock width based on start translation and minimum/maximum width for this translation
    ...
    // [Part 8] Lock actual OuterRect/WorkRect right-most translation.
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    // get the new cursor translation.
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    // Start translation is roughly ~~ CellRect.Min + CellPadding + Indent
...
void ImGui::TableEndCell(ImGuiTable* table)
{
    ...
    // Report maximum translation so we can infer content size per column.
    float* p_max_pos_x;
    if (table->RowFlags & ImGuiTableRowFlags_Headers)
...
// based on its translation (within frozen rows/columns groups or not).
