...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_HoveredRect        = 1 << 0,   // Mouse translation is within item rectangle (does NOT mean that the window is in correct z-order and can be hovered!, this is only one part of the most-common IsItemHovered test)
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    bool                    CursorFollow;           // set when we want scrolling to follow the current cursor translation (not always!)
...
struct ImGuiPopupData
{
    ...
    ImVec2              OpenPopupPos;   // Set on OpenPopup(), preferred popup translation (typically == OpenMousePos when using mouse)
    ImVec2              OpenMousePos;   // Set on OpenPopup(), copy of mouse translation at the time of opening popup
...
struct ImGuiListClipperRange
{
    int     Min;
    int     Max;
    bool    PosToIndexConvert;      // Begin/End are absolute translation (will be converted to indices later)
...
struct ImGuiContext
{
    ...
    bool                    NavMousePosDirty;                   // When set we will update mouse translation if (io.ConfigFlags & ImGuiConfigFlags_NavEnableSetMousePos) if set (NB: this not enabled by default)
...
{
    // Layout
    ImVec2                  CursorPos;              // Current emitting translation, in absolute coordinates.
    ImVec2                  CursorPosPrevLine;
    ImVec2                  CursorStartPos;         // Initial translation after Begin(), generally ~ window translation + WindowPadding.
    ...
    bool                    IsSameLine;
    bool                    IsSetPos;
    ImVec1                  Indent;                 // Indentation / start translation from left of window (increased by TreePush/TreePop, etc.)
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2                  Scroll;
    ImVec2                  ScrollMax;
    ImVec2                  ScrollTarget;                       // target scroll translation. stored as cursor translation with scrolling canceled out, so the highest point is always 0.0f. (FLT_MAX for no change)
    ImVec2                  ScrollTargetCenterRatio;            // 0.0f = scroll so that target translation is at top, 0.5f = scroll so that target translation is centered
    ...
    ImVec2                  SetWindowPosVal;                    // store window translation when using a non-zero Pivot (translation set needs to be processed when we know the window size)
    ...
    float                   FontWindowScale;                    // User scaling multiplier per-window, via SetWindowFontScale()
...
public:
    ...
    float       CalcFontSize() const    { ImGuiContext& g = *GImGui; float scaling = g.FontBaseSize * FontWindowScale; if (ParentWindow) scaling *= ParentWindow->FontWindowScale; return scaling; }
...
struct ImGuiTableColumn
{
    ...
    float                   WorkMinX;                       // Contents region min ~(MinX + CellPaddingX + CellSpacingX1) == cursor start translation when entering column
    float                   WorkMaxX;                       // Contents region max ~(MaxX - CellPaddingX - CellSpacingX2)
    float                   ItemWidth;                      // Current item width for the column, preserved across rows
    float                   ContentMaxXFrozen;              // Contents maximum translation for frozen rows (apart from headers), from which we can infer content width.
    float                   ContentMaxXUnfrozen;
    float                   ContentMaxXHeadersUsed;         // Contents maximum translation for headers rows (regardless of freezing). TableHeader() automatically softclip itself + report ideal desired size, to avoid creating extraneous draw calls
...
struct IMGUI_API ImGuiTable
{
    ...
    float                       RefScale;                   // Reference scaling to be able to rescale columns on font/dpi changes.
...
//#endif
    ...
    // NB: All translation are in absolute pixels coordinates (we are never using window coordinates internally)
    ...
    IMGUI_API void          RenderMouseCursor(ImVec2 pos, float scaling, ImGuiMouseCursor mouse_cursor, ImU32 col_fill, ImU32 col_border, ImU32 col_shadow);

    // Render helpers (those functions don't access any ImGui state!)
    IMGUI_API void          RenderArrow(ImDrawList* draw_list, ImVec2 pos, ImU32 col, ImGuiDir dir, float scaling = 1.0f);
